...
public class ExceptionAdvice {
    ...
    @ResponseBody
    @ResponseStatus(HttpStatus.INTERNAL_SERVER_ERROR)  //规定响应的状态的码为500
    @ExceptionHandler(Exception.class)
    public String exceptionHandler(Exception e) {
        JSONObject json = new JSONObject();
        //分类看是什么类型的异常，然后将异常放在json对象里
        if (e instanceof HttpMessageNotReadableException) {
            HttpMessageNotReadableException exception = (HttpMessageNotReadableException) e;  //强制类型转换
    ...
    @ResponseBody
    @ResponseStatus(HttpStatus.UNAUTHORIZED)   //规定响应的状态的码为403
    @ExceptionHandler(NotLoginException.class)
    public String unLoginHandler(Exception e) {   //捕获未登录异常
        JSONObject json = new JSONObject();
