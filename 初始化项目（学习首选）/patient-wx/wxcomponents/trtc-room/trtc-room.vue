<template>
<uni-shadow-root class="trtc-room-trtc-room"><view :class="'trtc-room-container '+(isFullscreenDevice?'fullscreen-device-fix':'')">
  <block v-if="template === '1v1'">
    <1v1 v-bind="{pusher, streamList, debug, enableIM}" wx-template-name="1v1"></1v1>
  </block>
  <block v-if="template === 'grid'">
    <grid v-bind="{pusher, streamList, visibleStreamList, debug, enableIM, panelName, gridPagePlaceholderStreamList, gridPageCount, gridCurrentPage, gridPlayerPerPage, isShowMoreMenu, MICVolume, BGMVolume, BGMProgress, beautyStyle, beautyStyleArray, filterIndex, filterArray, audioReverbTypeArray}" wx-template-name="grid"></grid>
  </block>
  <block v-if="template === 'custom'">
    <custom v-bind="{pusher, streamList, debug}" wx-template-name="custom"></custom>
  </block>
  ...
  <view :class="'debug-info '+(debugMode && debugPanel?'':'none')">
    ...
    <view v-for="(item,index) in (userList)" :key="item.userID">{{item.userID}}|
...
const __wxTemplateComponentProps = {"1v1":["wxTemplateName","pusher","streamList","debug","enableIM"],"grid":["wxTemplateName","pusher","streamList","visibleStreamList","debug","enableIM","panelName","gridPagePlaceholderStreamList","gridPageCount","gridCurrentPage","gridPlayerPerPage","isShowMoreMenu","MICVolume","BGMVolume","BGMProgress","beautyStyle","beautyStyleArray","filterIndex","filterArray","audioReverbTypeArray"],"custom":["wxTemplateName","pusher","streamList","debug"]}
import __wxTemplateComponent0 from './template/1v1/1v1.vue'
import __wxTemplateComponent1 from './template/grid/grid.vue'
import __wxTemplateComponent2 from './template/custom/custom.vue'
__wxTemplateComponentProps['1v1'] && __wxTemplateComponentProps['1v1'].forEach(prop => __wxTemplateComponent0.props[prop] = {type: null})
__wxTemplateComponentProps['grid'] && __wxTemplateComponentProps['grid'].forEach(prop => __wxTemplateComponent1.props[prop] = {type: null})
__wxTemplateComponentProps['custom'] && __wxTemplateComponentProps['custom'].forEach(prop => __wxTemplateComponent2.props[prop] = {type: null})
global['__wxVueOptions'] = {components:{,'1v1' : __wxTemplateComponent0,'grid' : __wxTemplateComponent1,'custom' : __wxTemplateComponent2}}
